...
namespace premier.parser
{
    ...
    public class tipico
    {
        ...
        public void PrepareHtmlProda(HtmlDocument document)
        {
            ...
            {
                //find headers for sports
                string sport;
                if (div.GetAttribute("className") != null && div.GetAttribute("className") == "")
                {
                        ...
                        else
                        {
                            ...
                            if (elemets.Count != 0)
                            {
                                if (string.IsNullOrEmpty(elemets[0]))
                                    for (int i = 0; i < 3; ++i)
                                        if (elemets[0]==null || !elemets[0].StartsWith("Next"))
                                            elemets.RemoveAt(0);
