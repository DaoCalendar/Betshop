...
return {
	/**
	 * wenn man an die URL ?debug anhÃ¤ngt werden debug-Meldungen ausgegeben
	 */
	...
	// Erklärung in #15822
	// wird zuerst im Web per Feature auf false gesetzt, soll danach auch auf Infoscreen genauso umgesetzt werden
	dontReplaceNext: false,
	// beim ersten Update wird ein "complete"-Request durchgefÃ¼hrt,
	// da es dann noch keine bestehenden Inhalte geben kann
	firstUpdate: true,
	compFilterEnabled: false,
	useCacheKey: false,
	// nur beim Laden ausgeführt
	...
	// URLs fÃ¼r normale Update-Requests und "complete"-Requests
	urlUpdate: '/spring/update',
	urlComplete: '/spring/complete',

	/**
	 * Liefert eine ID fÃ¼r den Ã¼bergebenen Knoten.
	 */
	...
	/**
	 * Liefert den e:hash-Wert vom Ã¼bergebenen Node, oder "_" falls der Node kein Hash hat
	 */
	...
	/**
	 * Liefert den e:url-Wert vom Ã¼bergebenen Node.
	 */
	...
	/**
	 * liefert die Update-URL fÃ¼r den Ã¼bergebenen Node (hash=escaped_url).
	 */
	...
	/**
	 * liefert die Update-URL fÃ¼r den Ã¼bergebenen key und value.
	 */
	encodeUrl: function(key, value) {
		// der Value wird escaped, da der HttpServletRequest den Wert "unescapet" bevor er ihn zurÃ¼ckliefert
		value = escape(value);
		
		// in der URL kÃ¶nnen Plus-Zeichen (fÃ¼r Space) vorkommen, da der Java URLEncoder dies so macht
		// diese mÃ¼ssen durch "%20" ersetzt werden, damit das escape/unescape
		// (hier unten und dann am am Server) funktioniert 
		value = value.replace(/[\+ ]/g, "%20");

		// nochmal escapen, da der Wert er als URL-Param zum Server Ã¼bertragen wird
		return key + "=" + escape(value);
	},
	/**
	 * Liefert den e:interval-Wert vom Ã¼bergebenen Node oder 0, falls kein Interval angegeben wurde.
	 */
	giveInterval: function(node) {
		return node.attr("e:interval") * 1 || 0;
	},
	/**
	 * Liefert den Timestamp, wann fÃ¼r den Node das nÃ¤chste mal ein Update gemacht werden muss.
	 */
	...
	/**
	 * Berechnet den Timestamp, wann der Node das nÃ¤chste mal geupdatet werden muss.
	 */
	...
	start: function() {
		if (RefreshHandler._interval !== undefined)
			return;
		RefreshHandler.run();
		RefreshHandler._interval = window.setInterval(function(){RefreshHandler.run()}, RefreshHandler.refreshInterval || 2000);
	},
	stop: function() {
		if (RefreshHandler._interval === undefined)
			return;
	...
	/**
	 * wird regelmÃ¤ÃŸig per Timer aufgerufen
	 */
	run: function() {
	...
	requestNodeUpdates: function(complete, updates, selector) {
		...
		selector.each(function(ix, node) {
			...
			// per JSF ausgetauscht wurde, daher mÃ¼ssen wir alle Nodes durchgehen
	...
	/**
	 * fÃ¼hrt einen complete-Request fÃ¼r die Ã¼bergebene Url aus.
	 * Die Orte, wo die Response-Inhalte eingefÃ¼gt werden, mÃ¼ssen zuvor per "e:comp-ref"
	...
	/**
	 * setzt aus den einzelURLs eine gesamtURL zusammen und führt
	...
	/**
	 * führt einen update-Request für die übergebenen Urls aus.
	 */
	update: function(){
		RefreshHandler.completeOrUpdate(RefreshHandler.urlUpdate, arguments);	
	},
	/**
	 * führt einen complete-Request für die übergebenen Urls aus.
	 */
	complete: function(){
	...
	/**
	 * fÃ¼hrt einen Server-Request aus
	 */
	requestUpdates: function(location, updates, navCall, doneCallback) {
		...
		if (RefreshHandler._delayedStop) {
			setTimeout(function() {RefreshHandler._norefresh = true;}, 5000);
		}
	...
	handleResponse: function(data) {
		...
		// wenn etwas ausgetauscht wurde mÃ¼ssen u.U. neue Inhalte angefordert werden
		if (count > 0)
			RefreshHandler.run();
		RefreshHandler._runOnComplete();
	},
	/**
	 * wird fÃ¼r jeden DOM-Node im Response einmal aufgerufen
	 */
	handleResponseNode: function(node) {
		...
		// script-Nodes werden nicht unterstÃ¼tzt
		if (node.is("script")) {
		...
		if (node.attr("e:url") === undefined) {
			// es gibt kein e:url, es kÃ¶nnte ein container-Objekt sein
		...
		jQuery(".e_active", node).each(function() {
			...
			if (url !== undefined && url.length != 0) {
				...
				if (existing.length) {
					...
					if(v1 != v2) {
						// #12755, ändert sich eine e:url in dem node, der mit @id versehen ist
						// soll der inhalt wie gewöhnt prozessiert werden, die e:url aber auf den neuen Wert geändert werden
	...
	/**
	 * Ausführen von Code, nach dem ein Request vollständig bearbeitet wurde.
	 */
	_onComplete: [],
	...
	/**
	 * Ausführen von Code wid _onComplete. Die Callback-Funktionen werden aber nicht entfernt
	 * */
	_onCompletePreserve: [],
